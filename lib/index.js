...
async function searchBase(dataRoot, input, scanner = createRipgrepScanner()) {
  const request = normalizeSearchRequest(input);
  const result = request.mode === "initial" ? await searchInitial(dataRoot, request, scanner) : await searchContinue(dataRoot, request.cursor, request.limit, scanner);
  await markUsed(dataRoot, result.baseId);
  return result;
}
async function searchInitial(dataRoot, request, scanner) {
...
async function searchContinue(dataRoot, cursor, limit, scanner) {
