...
function parseCard(value) {
  ...
  return {
    ...
    lastDestCategory: typeof record.lastDestCategory === "string" ? record.lastDestCategory : void 0
  };
}
...
async function createBase(dataRoot, input) {
  ...
  const catalog = await readCatalog(dataRoot);
  if (catalog.bases.some((card2) => card2.id === id) || await directoryExists(baseDir(dataRoot, id))) {
...
async function updateBase(dataRoot, id, patch) {
  ...
  const card = {
    ...currentCard,
    title: patch.title !== void 0 ? requireNonEmptyText(patch.title, "title") : currentCard.title,
