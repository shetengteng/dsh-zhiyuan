...
// src/model/package-info.ts
var PACKAGE_NAME = "dsh-zhiyuan";
var PACKAGE_VERSION = package_default.version;
var VERSION_LABEL = `v${PACKAGE_VERSION}`;
var TARGET_DSH_VERSION = package_default.dsh.targetVersion;
var COMMAND_NAME = "kb";
...
var VERSION_LABEL2 = `v${PACKAGE_VERSION2}`;
var TARGET_DSH_VERSION2 = package_default.dsh.targetVersion;
...
// src/model/csv-limits.ts
var CSV_MAX_PHYSICAL_LINE_BYTES2 = 64 * 1024;
var CSV_MAX_IMPORT_BYTES2 = 20 * 1024 * 1024;
var CSV_PREVIEW_MAX_CHARS = 2e5;
var CSV_PREVIEW_MAX_BYTES = CSV_MAX_IMPORT_BYTES2;
...
function hasOverlongPhysicalLine(bytes) {
  ...
  for (let index = 0; index < bytes.length; index += 1) {
    ...
    if (lineContentBytes(bytes, lineStart, index, lineNumber) > CSV_MAX_PHYSICAL_LINE_BYTES2) return true;
    lineStart = index + 1;
    lineNumber += 1;
  }
  return lineContentBytes(bytes, lineStart, bytes.length, lineNumber) > CSV_MAX_PHYSICAL_LINE_BYTES2;
}
...
function validateUtf8CsvBytes(bytes, maxBytes) {
  ...
  if (hasOverlongPhysicalLine(bytes)) {
    return { ok: false, code: "csv_line_too_long", message: `CSV \u5355\u884C\u4E0D\u80FD\u8D85\u8FC7 ${CSV_MAX_PHYSICAL_LINE_BYTES2} \u5B57\u8282` };
  }
...
async function prepareCsvImport(context) {
  const validation = await readNormalizedImportCsv(context.sourcePath, Math.min(context.maxFileBytes, CSV_MAX_IMPORT_BYTES2));
...
var SEARCH_RG_MAX_STDOUT_BYTES2 = 2 * 1024 * 1024;
...
async function readCsvPage(context) {
  const { document, revision } = await readCsvDocument(context.absolutePath, CSV_MAX_IMPORT_BYTES2);
...
function requireTableCellValue(data) {
  ...
  if (typeof value !== "string" || Buffer.byteLength(value, "utf8") > CSV_MAX_PHYSICAL_LINE_BYTES2) {
...
async function writeCsvDocument(context, document) {
  const maxFileBytes = Math.min(CSV_MAX_IMPORT_BYTES2, context.maxFileBytes);
...
// src/controller/rpc/knowledge-rpc-route.ts
...
function sendResult(res, rpcId, result) {
...
function endpointFromUrl(rawUrl) {
  ...
  if (!pathname.startsWith(`${KNOWLEDGE_RPC_CHANNEL}/`)) return void 0;
  const endpoint = pathname.slice(KNOWLEDGE_RPC_CHANNEL.length + 1);
  const segments = endpoint.split("/");
  if (segments.some((segment) => !ENDPOINT_SEGMENT_PATTERN.test(segment))) return void 0;
  return endpoint;
}
...
function registerKnowledgeRpcRoute(webServer, fence, handler) {
  ...
  const offs = [
    ctx.tools.register({
      ...
      output: {
        ...
        render: (_args, value) => {
          const kbs = value?.kbs ?? [];
          return [{ type: "text", text: kbs.map((item) => `${item.id} ${item.title.trim() || item.id}`).join(" \xB7 ") || "\u8FD8\u6CA1\u6709\u77E5\u8BC6\u5E93" }];
        }
      },
...
// src/service/kb/import/dropped-bytes-import.ts
import { mkdtemp, rm as rm6, writeFile as writeFile5 } from "node:fs/promises";
import { tmpdir } from "node:os";
import { join as join10 } from "node:path";

// src/service/kb/import/import-service.ts
...
// src/service/kb/import/import-naming.ts
...
// src/service/kb/import/import-destination.ts
...
// src/service/kb/import/import-source-scan.ts
...
// src/service/kb/import/import-entry-writer.ts
...
// src/service/kb/import/import-job.ts
...
function runRg(binaryPath, args, workingDirectory) {
  return new Promise((resolve2, reject) => {
    ...
    child.stdout.on("data", (chunk) => {
      ...
      if (stdoutBytes > SEARCH_RG_MAX_STDOUT_BYTES2 && !stopReason) {
...
// src/service/search/search-kb.ts
...
// src/skills/zhiyuan-kb/index.ts
...
function apply(ctx) {
  ...
  ctx.inject(["connection", "webServer"], (scoped) => {
    track(registerKnowledgePrivateRpc(scoped, jobs, knowledgeServices));
  });
