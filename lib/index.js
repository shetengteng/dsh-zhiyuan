...
function parseCard(value) {
  ...
  return {
    ...
    lastDestCategory: typeof rec.lastDestCategory === "string" ? rec.lastDestCategory : void 0
  };
}
...
import { basename, dirname as dirname2, extname, isAbsolute as isAbsolute2, join as join4, relative as relative3, sep as sep3 } from "node:path";
...
function winArgs(kind) {
  const script = kind === "dir" ? "Add-Type -AssemblyName System.Windows.Forms; $d = New-Object System.Windows.Forms.FolderBrowserDialog; $d.Description = '\u9009\u62E9\u8981\u5BFC\u5165\u7684\u6587\u4EF6\u5939'; if ($d.ShowDialog() -eq 'OK') { $d.SelectedPath }" : "Add-Type -AssemblyName System.Windows.Forms; $d = New-Object System.Windows.Forms.OpenFileDialog; $d.Filter = 'Markdown/Text|*.md;*.txt;*.markdown|All|*.*'; $d.Title = '\u9009\u62E9\u8981\u5BFC\u5165\u7684\u6587\u4EF6'; if ($d.ShowDialog() -eq 'OK') { $d.FileName }";
...
function parseRg(stdout, rootDir) {
  ...
  for (const raw of stdout.split(/\r?\n/)) {
    ...
    if (!hit) continue;
    const printed = hit[1];
    const abs = isAbsolute3(printed) ? printed : join5(rootDir, printed);
...
async function resolveIngestTo(dataRoot, baseId, destFlag, toRoot) {
  if (destFlag !== void 0) return destFlag;
  if (toRoot) return "";
