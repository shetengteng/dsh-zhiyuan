...
var COMMAND_NAME = "kb";
function formatBaseDisplayTitle(title, fallback = "") {
...
import { mkdir as mkdir6, stat as stat5 } from "node:fs/promises";
import { basename as basename5, dirname as dirname6, sep as sep5 } from "node:path";

// src/service/kb/bases.ts
import { randomUUID as randomUUID2 } from "node:crypto";
import { mkdir as mkdir2, readdir, rm as rm2, stat } from "node:fs/promises";
...
async function deleteBase(dataRoot, id, confirm) {
  ...
  await withCatalogTx(dataRoot, async ({ catalog }) => {
    ...
    await rm2(targetBaseDirectory, { recursive: true, force: true });
...
import { mkdir as mkdir3, rename as rename2, rm as rm3, writeFile as writeFile2 } from "node:fs/promises";
...
async function writeCsvDocument(context, document) {
  ...
  const temporaryPath = join4(entryDirectory, `.${basename2(context.absolutePath)}.${randomUUID3()}.tmp`);
  await mkdir3(entryDirectory, { recursive: true });
  try {
...
async function prepareMarkdownImport(context) {
  const byteLength = (await stat2(context.sourcePath)).size;
...
async function writeMarkdownContent(context) {
  ...
  await mkdir4(dirname3(context.absolutePath), { recursive: true });
...
async function walkSource(source) {
  ...
  for (const entry of entries) {
    const entryPath = join5(source, entry.name);
...
async function existingHashes(baseRoot) {
  ...
  for (const file of files) {
    ...
    map.set(await sha256File(file), relative2(baseRoot, file).split(sep2).join("/"));
  }
  return map;
}
...
import { basename as basename4, dirname as dirname5, join as join7, relative as relative4, sep as sep4 } from "node:path";

// src/content/shared/ingest-output.ts
import { randomUUID as randomUUID4 } from "node:crypto";
import { copyFile, rename as rename3, rm as rm4, stat as stat4, writeFile as writeFile4 } from "node:fs/promises";
async function writePreparedEntry(destinationPath, entry) {
  ...
  try {
    ...
    const writtenBytes = (await stat4(temporaryPath)).size;
...
async function ingestPrepared(args, name2, sourceRelativePath, failed, prepared, currentBytes) {
  ...
  const intendedPath = join7(args.destinationAbsolute, outputRelativePath(sourceRelativePath, name2, prepared.outputName));
  assertInside(args.baseRoot, intendedPath);
  assertNoSymlinkEscape(args.baseRoot, dirname5(intendedPath));
  await mkdir5(dirname5(intendedPath), { recursive: true });
  let destinationPath = intendedPath;
  let status = "copied";
  if (existsSync4(destinationPath)) {
    destinationPath = join7(dirname5(intendedPath), uniqueName(dirname5(intendedPath), basename4(intendedPath)));
    status = "renamed";
  }
  const writtenBytes = await writePreparedEntry(destinationPath, prepared);
  const relativeDestinationPath = relative4(args.baseRoot, destinationPath).split(sep4).join("/");
...
async function importFiles(dataRoot, input) {
  ...
  const sourceInfo = await stat5(source);
  ...
  for (const file of files) {
    ...
    for (const fileResult of fileResults) {
      ...
      if (fileResult.status === "skipped") result.skipped += 1;
        ...
        if (fileResult.relPath.includes("/")) createdDirs.add(dirname6(fileResult.relPath).split(sep5).join("/"));
...
function cursorQueryFromSearch(query, baseId, category, path) {
  return {
    baseId,
    terms: [...query.terms],
...
function isValidPayload(value) {
  ...
  const query = asRecord2(record.query);
  if (!query || typeof query.baseId !== "string" || !query.baseId.trim() || !isStringArray(query.terms) || !query.terms.length || !isStringArray(query.aliases)) return false;
  ...
  if (record.scope === "files") {
    ...
    const position2 = asRecord2(record.position);
  ...
  const position = asRecord2(record.position);
...
async function searchFileDetail(scope, hitIndex, limit, scanner) {
  ...
  let document;
  if (matches.length) {
    const absolutePath = assertInside(scope.rootDir, join8(scope.rootDir, ...scope.path.split("/")));
...
function optionalString(record, key) {
  if (!hasOwn(record, key) || record[key] === void 0) return void 0;
...
import { isAbsolute as isAbsolute3, join as join9, relative as relative5, sep as sep6 } from "node:path";
...
function resolveCategory(rootDir, dataRoot, baseId, category) {
...
function resolveKnowledgePath(rootDir, categoryRoot, inputPath) {
  ...
  const absolutePath = assertInside(rootDir, join9(rootDir, ...segments));
  const normalizedPath = relative5(rootDir, absolutePath).split(sep6).join("/");
...
import { isAbsolute as isAbsolute4, join as join10, relative as relative6, sep as sep7 } from "node:path";
...
function parseRg(stdout, rootDir) {
  ...
  for (const rawLine of stdout.split(/\r?\n/u)) {
    ...
    const pathData = asRecord4(data?.path);
    ...
    const firstSubmatch = asRecord4(submatches[0]);
    ...
    const absolutePath = isAbsolute4(printedPath) ? printedPath : join10(rootDir, printedPath);
    const relativePath = relative6(rootDir, absolutePath).split(sep7).join("/");
...
async function searchBase(dataRoot, input, baseAccess, scanner = createRipgrepScanner()) {
  const request = normalizeSearchRequest(input);
  const result = request.mode === "initial" ? await searchInitial(dataRoot, request, baseAccess, scanner) : await searchContinue(dataRoot, request.cursor, request.limit, baseAccess, scanner);
  await baseAccess.markBaseUsed(result.baseId);
  return result;
}
async function searchInitial(dataRoot, request, baseAccess, scanner) {
...
async function searchContinue(dataRoot, cursor, limit, baseAccess, scanner) {
  ...
  if (payload.scope === "files") {
    const scope2 = await resolveSearchScope(dataRoot, { baseId: payload.query.baseId, query, category: payload.query.category });
...
function addDetailCursor(result, nextHitIndex) {
  ...
  const cursor = encodeSearchCursor({
    version: 3,
    scope: "hits",
    query: cursorQueryFromSearch(result.query, result.baseId, result.category, result.path),
    position: { hitIndex: nextHitIndex }
  });
  return { ...result, page: { ...result.page, nextCursor: cursor } };
}

// src/controller/search-operation.ts
function createSearchBaseAccess(dataRoot) {
  return {
    ensureBase: (baseId) => requireBase(dataRoot, baseId),
    markBaseUsed: (baseId) => markUsed(dataRoot, baseId)
  };
}
async function searchKnowledgeBase(dataRoot, input) {
  return searchBase(dataRoot, input, createSearchBaseAccess(dataRoot));
}

// src/controller/command-parse.ts
...
// src/service/kb/base-tree.ts
import { readdir as readdir3, stat as stat6 } from "node:fs/promises";
import { join as join11, relative as relative7, sep as sep8 } from "node:path";
async function walkTextDocuments(directoryPath) {
  const documentPaths = [];
  let entries;
  try {
    entries = await readdir3(directoryPath, { withFileTypes: true });
  } catch {
    return documentPaths;
  }
  for (const entry of entries) {
    const entryPath = join11(directoryPath, entry.name);
...
async function textDocumentBytes(baseRoot) {
  ...
  let total = 0;
  for (const documentPath of paths) total += (await stat6(documentPath)).size;
  return total;
}
async function listBaseCategories(dataRoot, baseId) {
  ...
  const entries = await readdir3(baseDirectory, { withFileTypes: true });
  return entries.filter((entry) => entry.isDirectory() && !entry.name.startsWith(".")).map((entry) => entry.name);
}
...
async function walkTree(baseRoot, directoryPath) {
  ...
  for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name, "zh"))) {
    const absolutePath = join11(directoryPath, entry.name);
    const relativePath = relative7(baseRoot, absolutePath).split(sep8).join("/");
...
async function fileBytes(filePath) {
  try {
    return (await stat7(filePath)).size;
  } catch (error) {
...
async function deleteEntry(dataRoot, baseId, relativePath, confirm) {
  ...
  await rm5(absolutePath, { recursive: true, force: true });
}
...
async function importDroppedBytes(dataRoot, input) {
  const fileName = sanitizeDroppedFileName(input.fileName);
  if (input.bytes.length === 0) throw new KbError("invalid_field", "\u62D6\u5165\u6587\u4EF6\u662F\u7A7A\u7684");
  const tempDir = await mkdtemp(join12(tmpdir(), "zy-drop-"));
  const sourcePath = join12(tempDir, fileName);
  try {
    await writeFile5(sourcePath, input.bytes, { flag: "wx" });
    return await importFiles(dataRoot, buildImportInput({
      baseId: input.baseId,
      sourcePath,
      destCategory: input.destCategory,
...
function optionalString2(data, field) {
...
function readPreviewOptions(data) {
  ...
  const sourceFingerprint = optionalString2(data, "sourceFingerprint");
...
// src/controller/rpc-dispatch.ts
...
async function setPrefs(dataRoot, data) {
  return withCatalogTx(dataRoot, async ({ catalog }) => {
    const defaultBaseId = optionalString2(data, "defaultBaseId");
  ...
  switch (operation) {
    ...
    case "update":
      return updateBase(dataRoot, requireString(data, "id"), {
        title: optionalString2(data, "title"),
        description: optionalString2(data, "description"),
        aliases: optionalStringArray(data, "aliases")
      });
    case "deleteBase":
    ...
    case "import": {
      const sourceBase64 = optionalString2(data, "sourceBase64");
    ...
    case "search": {
      if (hasField(data, "cursor")) {
        return searchKnowledgeBase(dataRoot, {
          ...
          ...optionalPositiveInteger(data, "limit") === void 0 ? {} : { limit: optionalPositiveInteger(data, "limit") }
        });
      }
      return searchKnowledgeBase(dataRoot, {
        baseId: requireString(data, "baseId"),
        ...
        aliases: optionalStringArray(data, "aliases"),
        category: optionalString2(data, "category"),
        path: optionalString2(data, "path"),
        ...optionalPositiveInteger(data, "limit") === void 0 ? {} : { limit: optionalPositiveInteger(data, "limit") }
      });
    }
    case "prefs":
...
async function resolveImportTo(dataRoot, baseId, destinationCategoryFlag, importToBaseRoot) {
...
function buildSearchRequest(rest, flags) {
  ...
  if (cursor) return { cursor, ...limit === void 0 ? {} : { limit } };
  return {
...
async function handleImport(rest, flags, jobs) {
...
function renderImportResult(value) {
  ...
  const failedFiles = files.map((item) => asRecord7(item)).filter((item) => item !== null && item.status === "failed").slice(0, 5).map((item) => `${typeof item.sourceRelPath === "string" ? item.sourceRelPath : String(item.relPath ?? "\u6587\u4EF6")}\uFF1A${typeof item.reason === "string" ? item.reason : "\u5904\u7406\u5931\u8D25"}`);
  const summary = `\u5BFC\u5165 ${copied.length} \xB7 \u8DF3\u8FC7 ${skipped} \xB7 \u5931\u8D25 ${failed}`;
  return text(failedFiles.length ? `${summary}
${failedFiles.join("\n")}` : summary);
}

// src/controller/tools.ts
    ...
    ctx.tools.register({
      ...
      output: {
        ...
        render: (_args, value) => {
          ...
          return [{ type: "text", text: bases.map((item) => `${item.id} ${formatBaseDisplayTitle(item.title, item.id)}`).join(" \xB7 ") || "\u8FD8\u6CA1\u6709\u77E5\u8BC6\u5E93" }];
        }
      },
...
function apply(ctx) {
  ...
  ctx.inject(["connection"], (scoped) => {
    trackAsync(registerKnowledgePrivateRpc(scoped, jobs));
  });
