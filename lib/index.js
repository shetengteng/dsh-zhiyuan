...
function readPreviewOptions(data) {
  ...
  if (data.matchColumnByte !== void 0 && !positiveInteger(data.matchColumnByte)) {
    throw new KbError("invalid_preview", "\u641C\u7D22\u9884\u89C8\u547D\u4E2D\u5217\u65E0\u6548");
  }
  if (data.sourceFingerprint !== void 0 && (typeof data.sourceFingerprint !== "string" || data.sourceFingerprint.length > 128)) {
    throw new KbError("invalid_preview", "\u641C\u7D22\u9884\u89C8\u6587\u4EF6\u6307\u7EB9\u65E0\u6548");
  }
  return {
    view,
    matchLine: data.matchLine,
    matchColumnByte: data.matchColumnByte,
...
function renderIngestResult(value) {
  ...
  const failedFiles = files.map((item) => asRecord2(item)).filter((item) => Boolean(item) && item.status === "failed").slice(0, 5).map((item) => `${typeof item.sourceRelPath === "string" ? item.sourceRelPath : String(item.relPath ?? "\u6587\u4EF6")}\uFF1A${typeof item.reason === "string" ? item.reason : "\u5904\u7406\u5931\u8D25"}`);
    ...
    ctx.tools.register({
      ...
      output: {
        ...
        presentationMeta: (args, value) => {
          const baseId = asString2(asRecord2(args)?.baseId)?.trim();
          const result = asRecord2(value);
