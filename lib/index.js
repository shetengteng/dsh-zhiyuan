...
import { basename as basename2, dirname as dirname4, extname as extname2, isAbsolute as isAbsolute2, join as join5, relative as relative3, sep as sep3 } from "node:path";

// src/bases.ts
import { mkdir as mkdir4, readdir, rm as rm2, stat as stat2 } from "node:fs/promises";
...
import { mkdir as mkdir2, rename, rm, writeFile as writeFile2 } from "node:fs/promises";
import { basename, dirname as dirname2, join as join3 } from "node:path";
var UTF8_BOM = Buffer.from([239, 187, 191]);
async function writeCsvEntry(context) {
...
// src/content/csv/server/editor.ts
async function readCsvEditorPage(context) {
...
async function readCsvPreview(context) {
  ...
  const revision = createHash3("sha256").update(validation.value.bytes).digest("hex");
  const document = parseCsvDocument(text2);
  if (context.options.readMode === EntryReadMode.Edit) {
    const csv2 = createCsvEditorPage(document, 1, CSV_EDITOR_PAGE_SIZE, revision);
    const lastRecord = csv2.windowEndRow ? document.records[csv2.windowEndRow - 1] : document.header;
    return {
      ...
      truncation: csv2.complete ? "none" : "after",
  ...
  const previewCsv = focus.previewStatus === "ready" || window.csv.focusedRow === void 0 ? window.csv : { ...window.csv, focusedRow: void 0 };
  const csv = { ...previewCsv, revision };
  return {
    path: context.relativePath,
    text: text2.slice(window.textStartOffset, window.textEndOffset),
    format: EntryFormat.Csv,
    view: focus.view,
...
function createPhysicalLineSearchDocument(bytes, text2) {
  const lines = splitPhysicalLines(text2);
  const fingerprint = createHash4("sha256").update(bytes).digest("hex");
  return {
    fingerprint,
...
async function readMarkdownPreview(context) {
  ...
  const lines = splitPhysicalLines(text2);
  const focus = resolvePreviewFocus(lines, createHash6("sha256").update(bytes).digest("hex"), context.options);
  return {
    path: context.relativePath,
    text: text2,
    format: EntryFormat.Markdown,
...
async function writeMarkdownEntry(context) {
  ...
  await writeFile3(context.absolutePath, context.text, "utf8");
}
...
var contentRegistry = {
  ...
  readCsvEditorPage: async (context) => {
    const handler = entryHandlerForPath(context.relativePath);
...
async function walkTextDocuments(directoryPath) {
  ...
  for (const entry of entries) {
    const entryPath = join4(directoryPath, entry.name);
...
async function walkTree(baseRoot, directoryPath) {
  ...
  for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name, "zh"))) {
    const absolutePath = join4(directoryPath, entry.name);
...
async function readCsvEditorPage2(dataRoot, baseId, relativePath, startRow, pageSize2) {
  await requireBase(dataRoot, baseId);
  const absolutePath = resolveDest(dataRoot, baseId, relativePath).absolute;
  const baseRoot = baseDir(dataRoot, baseId);
  assertInside(baseRoot, absolutePath);
  assertNoSymlinkEscape(baseRoot, absolutePath);
  return contentRegistry.readCsvEditorPage({ absolutePath, relativePath, startRow, pageSize: pageSize2 });
}
async function writeCsvPatch(dataRoot, baseId, relativePath, patch) {
  await requireBase(dataRoot, baseId);
  const absolutePath = resolveDest(dataRoot, baseId, relativePath).absolute;
  const baseRoot = baseDir(dataRoot, baseId);
  assertInside(baseRoot, absolutePath);
...
async function deleteEntry(dataRoot, baseId, relativePath, confirm) {
  ...
  await rm2(absolutePath, { recursive: true, force: true });
}

// src/content/shared/ingest-output.ts
import { randomUUID as randomUUID3 } from "node:crypto";
import { copyFile, rename as rename2, rm as rm3, stat as stat3, writeFile as writeFile4 } from "node:fs/promises";
async function writePreparedEntry(destinationPath, entry) {
  const temporaryPath = `${destinationPath}.${randomUUID3()}.tmp`;
  try {
    ...
    await rename2(temporaryPath, destinationPath);
    return writtenBytes;
  } catch (error) {
...
function uniqueName(dir, name2) {
  ...
  const stem = basename2(name2, ext);
  let next = name2;
  let n = 2;
  while (existsSync3(join5(dir, next))) {
    next = `${stem}-${n}${ext}`;
    n += 1;
  }
  return next;
...
function relativeSourcePath(sourceRoot, file, preserveTree) {
  if (!preserveTree) return basename2(file);
...
function outputRelativePath(sourceRelativePath, sourceName, outputName) {
  ...
  return join5(dirname4(sourceRelativePath), outputName).split(sep3).join("/");
}
...
async function ingestOne(args) {
  const name2 = basename2(args.file);
...
async function ingestOneUnsafe(args, name2, sourceRelativePath, failed) {
  ...
  if (!prepared.outputName || basename2(prepared.outputName) !== prepared.outputName) {
    return failed("io_failed", "\u8F6C\u6362\u4EA7\u7269\u540D\u65E0\u6548");
  }
  const intendedPath = join5(args.destinationAbsolute, outputRelativePath(sourceRelativePath, name2, prepared.outputName));
  ...
  if (existsSync3(destinationPath)) {
    destinationPath = join5(dirname4(intendedPath), uniqueName(dirname4(intendedPath), basename2(intendedPath)));
    status = "renamed";
  }
...
import { isAbsolute as isAbsolute3, join as join6, relative as relative4, sep as sep4 } from "node:path";
...
function parseRg(stdout, rootDir) {
  ...
  for (const raw of stdout.split(/\r?\n/)) {
    ...
    const absolutePath = isAbsolute3(printedPath) ? printedPath : join6(rootDir, printedPath);
...
var RipgrepSearchEngine = class {
  async search(input) {
    ...
    for (const match of matches) {
      const absolutePath = join6(input.rootDir, match.path);
...
function requireCsvPatch(data) {
  const patch = asRecord2(data.patch);
...
function requireCsvHeaderChanges(data) {
...
function requireCsvPageSize(data) {
  const pageSize2 = optionalPositiveInteger(data, "pageSize") ?? CSV_EDITOR_PAGE_SIZE;
  if (pageSize2 > CSV_EDITOR_PAGE_SIZE) throw new KbError("csv_patch_invalid", `\u6BCF\u9875\u6700\u591A ${CSV_EDITOR_PAGE_SIZE} \u884C`);
  return pageSize2;
}
