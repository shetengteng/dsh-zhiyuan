...
// src/content/csv/server/patch-schema.ts
...
function assertCsvValue(value) {
  if (typeof value !== "string" || Buffer.byteLength(value, "utf8") > CSV_MAX_PHYSICAL_LINE_BYTES) {
    throw new KbError("csv_patch_invalid", "\u5355\u5143\u683C\u5185\u5BB9\u8FC7\u957F");
  }
}
function assertCsvPatchShape(patch) {
...
function parseCsvEntryPatch(value) {
...
async function writeCsvEntry(context) {
  const sourceBytes = encodeUtf8CsvWithBom(context.text);
...
function runRg(binaryPath, rgArgs, workingDirectory) {
  return new Promise((resolve2, reject) => {
    ...
    child.on("close", (code) => {
      ...
      if (code === 0 || code === 1 || timedOut || truncated) resolve2({ stdout, warnings });
...
function renderIngestResult(value) {
  ...
  const failedFiles = files.map((item) => asRecord4(item)).filter((item) => Boolean(item) && item.status === "failed").slice(0, 5).map((item) => `${typeof item.sourceRelPath === "string" ? item.sourceRelPath : String(item.relPath ?? "\u6587\u4EF6")}\uFF1A${typeof item.reason === "string" ? item.reason : "\u5904\u7406\u5931\u8D25"}`);
    ...
    ctx.tools.register({
      ...
      output: {
        ...
        presentationMeta: (args, value) => {
          const baseId = asString2(asRecord4(args)?.baseId)?.trim();
          const result = asRecord4(value);
