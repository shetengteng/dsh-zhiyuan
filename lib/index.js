...
async function searchFileDetail(scope, hitIndex, limit, scanner) {
  ...
  let document;
  if (matches.length) {
    const absolutePath = assertInside(scope.rootDir, join7(scope.rootDir, ...scope.path.split("/")));
...
// src/search/search-scope.ts
import { existsSync as existsSync4, statSync } from "node:fs";
import { isAbsolute as isAbsolute3, join as join8, relative as relative4, sep as sep4 } from "node:path";
...
function resolveCategory(rootDir, dataRoot, baseId, category) {
  ...
  if (!existsSync4(destination.absolute) || !statSync(destination.absolute).isDirectory()) {
...
function resolveKnowledgePath(rootDir, categoryRoot, inputPath) {
  ...
  const absolutePath = assertInside(rootDir, join8(rootDir, ...segments));
  const normalizedPath = relative4(rootDir, absolutePath).split(sep4).join("/");
  ...
  if (!existsSync4(absolutePath)) throw new KbError("not_found", `\u6587\u4EF6\u4E0D\u5B58\u5728\uFF1A${path}`);
  if (!statSync(absolutePath).isFile()) throw new KbError("invalid_field", "path \u5FC5\u987B\u6307\u5411\u6587\u4EF6");
  return normalizedPath;
}

// src/search/scanner/ripgrep-scanner.ts
import { spawn } from "node:child_process";
import { existsSync as existsSync5 } from "node:fs";
import { isAbsolute as isAbsolute4, join as join9, relative as relative5, sep as sep5 } from "node:path";
...
function buildArguments(terms, targetPath) {
  const args = [
    "--json",
    "--column",
...
function parseRg(stdout, rootDir) {
  ...
  for (const rawLine of stdout.split(/\r?\n/u)) {
    ...
    const absolutePath = isAbsolute4(printedPath) ? printedPath : join9(rootDir, printedPath);
...
async function resolveRg() {
  ...
  if (!ripgrepPath || !existsSync5(ripgrepPath)) throw new Error("\u627E\u4E0D\u5230\u6253\u5305\u7684 ripgrep");
  return ripgrepPath;
}
...
function renderIngestResult(value) {
  ...
  const failedFiles = files.map((item) => asRecord6(item)).filter((item) => item !== null && item.status === "failed").slice(0, 5).map((item) => `${typeof item.sourceRelPath === "string" ? item.sourceRelPath : String(item.relPath ?? "\u6587\u4EF6")}\uFF1A${typeof item.reason === "string" ? item.reason : "\u5904\u7406\u5931\u8D25"}`);
  const summary = `\u5BFC\u5165 ${copied.length} \xB7 \u8DF3\u8FC7 ${skipped} \xB7 \u5931\u8D25 ${failed}`;
  return text(failedFiles.length ? `${summary}
${failedFiles.join("\n")}` : summary);
}

// src/search/result/templates/file-detail-template.ts
...
// src/search/result/templates/overview-template.ts
...
// src/search/result/templates/index.ts
...
function optionalString3(args, key) {
  const value = args[key];
  if (value === void 0) return void 0;
...
function optionalStringArray2(args, key) {
  const value = args[key];
  if (value === void 0) return void 0;
  if (!Array.isArray(value) || value.some((item) => typeof item !== "string")) {
...
function buildSearchRequest2(input) {
  if (input.cursor !== void 0) {
    return {
      cursor: requireString2(input, "cursor"),
      ...optionalNumber(input, "limit") === void 0 ? {} : { limit: optionalNumber(input, "limit") }
    };
  }
  return {
    baseId: requireString2(input, "baseId"),
    query: requireString2(input, "query"),
    aliases: optionalStringArray2(input, "aliases"),
    category: optionalString3(input, "category"),
    path: optionalString3(input, "path"),
    ...optionalNumber(input, "limit") === void 0 ? {} : { limit: optionalNumber(input, "limit") }
  };
}
  ...
  const offs = [
    ...
    ctx.tools.register({
      ...
      execute: async (args) => {
        ...
        try {
          const dataRoot = await resolveDataRoot();
          return await searchBase(dataRoot, buildSearchRequest2(input));
        } catch (error) {
          fail2(error);
        }
      }
