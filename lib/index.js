...
async function readCsvPreview(context) {
  ...
  const window = createCsvPreviewWindow(
    parseCsvDocument(text2),
    context.options.readMode === EntryReadMode.Edit,
    focus.hasRequestedFocus ? focus.requestedLine : void 0
  );
  const csv = focus.previewStatus === "ready" || window.csv.focusedRow === void 0 ? window.csv : { ...window.csv, focusedRow: void 0 };
  return {
