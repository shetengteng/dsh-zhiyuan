import { CitationTag } from '../CitationTag.tsx'

export function AboutPage() {
  return (
    <div className="zy-about">
      <h3>这是什么</h3>
      <p>知源是 DSH 的本地优先知识库。它把你指定的 md/txt/csv 导入本机文件夹。CSV 会转成 UTF-8 再入库。提问时一次只查一个库。导入不会自动建库，也不会把文件默默塞进某个库。</p>
      <p>拔网线能搜。完整答案取决于有没有本地模型。</p>

      <h3>1. 建库</h3>
      <p>「知识库」页左下角或空态里的「新建知识库」。填写标题、描述、别名；标题不能重复，id 由系统自动生成且不在表单中展示。</p>
      <p>描述和别名是给选库用的：对话里没说查哪个库时，会拿各库的描述、别名对问题。两个都像就会问你，不会把所有库扫一遍。</p>
      <div className="zy-demo">
        <div className="zy-q">工作库 · 别名：工作, 公司, 合同库</div>
        <p className="zy-a">描述：公司合同、会议纪要、供应商往来。问条款、纪要、交付时用本库。个人账单、家庭、医疗不要放。</p>
      </div>

      <h3>2. 导入</h3>
      <ol>
        <li>先选好左侧的库。</li>
        <li>点右侧「导入」。</li>
        <li>把文件或文件夹拖到拖拽框，或点击框中的「选择文件夹」「选择文件」按钮打开系统选择器。</li>
        <li>指定类目，例如 <code>合同/2024</code>。没有这个文件夹会创建；不会因此新建一个知识库。</li>
        <li>开始导入。内容相同的文件会跳过，不会覆盖。目前支持 md/txt 和 csv（GBK / UTF-16 会转成 UTF-8）；CSV 可在预览弹框中表格编辑。</li>
      </ol>
      <p>斜杠命令 <code>/kb import</code> 与按钮同一套字段。</p>

      <h3>3. 用自然语言问</h3>
      <p>在对话里直接问，不必先打开设置。点名库更准；没点名就靠描述选库。</p>
      <div className="zy-demo">
        <div className="zy-q">去年那个供应商合同的违约条款写了什么？</div>
        <p className="zy-a">对上工作库 → 命中卡。点卡只读预览原文，该句高亮。</p>
      </div>
      <div className="zy-demo">
        <div className="zy-q">上个月水电费多少？</div>
        <p className="zy-a">对上生活库。工作库里的合同不会被打开。</p>
      </div>
      <div className="zy-demo">
        <div className="zy-q">用 work 库查 termination 条款</div>
        <p className="zy-a">已经点名库，不再猜。只搜 work。</p>
      </div>
      <p>命中卡可叠多条。点任意一条看原文。答案里的 <CitationTag n={1} /> 对应卡片编号。库里没有的内容不会编出来。</p>
    </div>
  )
}
