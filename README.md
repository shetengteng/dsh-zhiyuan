...
人显式新建知识库，把本机 `.md` / `.txt` / `.markdown` 拷进指定库和类目；查询时先选库，再用 3～8 个关键词一次 grep。命中带文件路径、行号和片段 `[n]`，交给当前对话模型写答案。

- **正文只在文件夹里。** 导入是拷贝，不是记外链，也不把全文写入数据库。
- **类目就是子文件夹。** 例如 `bases/work/合同/2024/供应商合同.md`。
...
库卡片字段：`id` / `title` / `description` / `aliases`。`id` 创建后不可改（`[a-z0-9][a-z0-9_-]{0,63}`）。

## 怎么用

1. **建库**：设置 → 知源 → 新建。必填 id、标题、描述；别名可选（如「工作」「公司」）。导入路径不会自动建库。
...
`--to` 缺则问一句，或用该库上次类目。不单独做 `/kb search` 当主路径。
...
对话里的命中卡展示路径、片段和 `[n]`；点开只读预览，不能改，不跳设置页。
...
1. 新建库 `work`，别名：工作、公司。描述写清「问条款、纪要开这个库；个人账单不要放」。
2. 导入本机一份合同 md，类目 `合同/2024`（原先可以不存在）。
3. 磁盘为 `bases/work/合同/2024/…`；源文件未被改。
4. 问违约条款：模型先列库再 `kb_search(baseId=work, …)`；命中带行号。
5. 拔网线：再导入、再试搜，仍成功。
