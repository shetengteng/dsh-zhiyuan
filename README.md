...
- **类目就是子文件夹。** 例如 `kbs/<uuid>/合同/2024/供应商合同.md`。
...
`catalog.json` 没有也能扫 `kbs/` 列出库；没有卡片时描述为空，模型容易选错库，所以创建入口必须写描述。
...
2. **导入**：指定已有 `kbId` 和类目 `destCategory`（空 = 库根）。缺类目则创建文件夹；缺库则失败。源文件只读，不改动。
3. **问**：自然语言问库里的事实。模型应先 `kb_list`，再一次 `kb_search`。没命中不得说「根据知识库」。
...
| 工具        | 作用                                                                    |
| ----------- | ----------------------------------------------------------------------- |
| `kb_list`   | 列库：id / 标题 / 描述 / 别名 / 类目名 / 约多少篇。不含文件名、不含正文 |
| `kb_import` | 拷进已有库。`kbId`、`sourcePath` 必填；`destCategory` 语义必填          |
| `kb_search` | 只扫指定库。不带 `kbId` 则校验失败。`aliases` 建议 3～8 个，一次 OR     |
...
3. 磁盘为 `kbs/<uuid>/合同/2024/…`；源文件未被改。
4. 问违约条款：模型先列库，再用返回的 `kbId` 调 `kb_search`；命中带行号。
5. 拔网线：再导入、再试搜，仍成功。

必须失败：import 到不存在的库；`destCategory` 含 `..` 或绝对路径；`kb_search` 不带 `kbId`；单文件超过 5 MB（该文件失败，其他可继续）；空库搜索返回空列表。

## 停用与卸载

从所用 profile 的依赖 / bundle 层中去掉本包后重启 Host。去掉后 Cordis 行 `zhiyuan` 不应再出现；Web 不再挂「知源」section。

卸载默认**不删除**插件数据目录里的 `kbs/` 与 `catalog.json`。若要清掉本地副本，再手动删除该数据目录。删库 / 删文件删的是知识库副本，不是当初指定的源路径。
