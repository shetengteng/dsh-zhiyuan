...
| `app/tools.ts`         | 注册 `kb_list`、`kb_import`、`kb_search`，瘦身为注册与装配                                                                                                                   |
...
| 新模块               | 建议职责                                                        |
| -------------------- | --------------------------------------------------------------- |
| `app/tools.ts`       | 注册 `kb_list`、`kb_import`、`kb_search`                        |
...
5. 外部命名统一（兼容性已放开）：工具 `kb_ingest` → `kb_import`，RPC op `'ingest'` → `'import'`，命令 `ingest <path>` → `import <path>`，JobStatus 的 op 标签同步改；`kb_list`、`kb_search` 不变。
...
- [ ] **T2.5 外部命名统一**：工具 `kb_ingest` → `kb_import`，op `'ingest'` → `'import'`，命令 `ingest <path>` → `import <path>`，JobStatus op 标签同步改；`kb_list`、`kb_search` 名称、参数结构、结果字段和错误行为不变。
