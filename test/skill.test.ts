...
test('skill 正文锁住检索流程、边界、出处和导入规则', () => {
  ...
  assert.match(body, /kb_list/)
  assert.match(body, /不要同时搜索两个库/)
  assert.match(body, /默认搜索全部 kbs/)
  assert.match(body, /aliases/)
  assert.match(body, /`kb_search` 首次查询必须带 `kbId` 和 `query`/)
  assert.match(body, /不能只传 `kbId\/path`/)
...
test('system prompt 段与 skill 同口径', () => {
  ...
  assert.match(ZHIYUAN_PROMPT_SECTION.text, /`kb_list`/)
  assert.match(ZHIYUAN_PROMPT_SECTION.text, /扫描全部 kbs/)
  ...
  assert.match(ZHIYUAN_PROMPT_SECTION.text, /不能只传 `kbId\/path`/)
})
