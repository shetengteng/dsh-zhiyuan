...
import { searchKb as searchKbWithAccess, type SearchRequest, type SearchScanner } from '../src/service/search/index.ts'
import { createRipgrepScanner } from '../src/service/search/scanner/index.ts'
...
const { createKb, markKbUsed, requireKb } = knowledgeServices
...
async function searchKb(
  ...
  return searchKbWithAccess(dataRoot, input, createSearchKbAccess(dataRoot), scanner)
}
...
test('ripgrep 正则编译失败返回稳定输入错误，而不是未完成扫描', async () => {
  await withKb('zy-search-rg-pattern-', { 'a.md': '违约。\n' }, async (root, kbId) => {
    await assert.rejects(
      () => createRipgrepScanner().scan({ rootDir: join(root, 'kbs', kbId), terms: ['(?=违约)'] }),
      (error: unknown) => error instanceof KbError
...
test('空库搜索返回 overview 空结果，不携带 hits', async () => {
  ...
  try {
    ...
    const result = asOverview(await searchKb(root, { kbId: kb.id, query: '违约' }))
...
test('输入校验拒绝缺字段、非法正则、别名和 limit', async () => {
  await assert.rejects(() => searchKb('/tmp', { kbId: '', query: '违约' }), KbError)
  await assert.rejects(() => searchKb('/tmp', { kbId: 'work', query: '  ' }), /query 必填/)
  await assert.rejects(() => searchKb('/tmp', { kbId: 'work', query: '(?=违约)' }), /不支持的正则语法/)
  await assert.rejects(() => searchKb('/tmp', { kbId: 'work', query: '违约', aliases: [''] }), /不能包含空正则/)
  await assert.rejects(() => searchKb('/tmp', { kbId: 'work', query: '违约', aliases: Array.from({ length: 9 }, (_, index) => `词${index}`) }), /不能超过 8 个/)
  await assert.rejects(() => searchKb('/tmp', { kbId: 'work', query: '违约', limit: 0 }), /limit 必须是/)
  await assert.rejects(() => searchKb('/tmp', { kbId: 'work', query: '违约', limit: 101 }), /limit 必须是/)
})

/** 建临时库并在 kbs/<id>/ 下写文件，结束后清理。 */
async function withKb(
  prefix: string,
  files: Record<string, string>,
  work: (root: string, kbId: string) => Promise<void>,
): Promise<void> {
  const root = await mkdtemp(join(tmpdir(), prefix))
  try {
    const kb = await createKb(root, { title: '工作库', description: '描述' })
    for (const [relativePath, body] of Object.entries(files)) {
      const target = join(root, 'kbs', kb.id, ...relativePath.split('/'))
      await mkdir(dirname(target), { recursive: true })
      await writeFile(target, body)
    }
    await work(root, kb.id)
  } finally {
    await rm(root, { recursive: true, force: true })
  }
}
...
async function searchAllOverviewPages(
  ...
  let request: SearchRequest = { kbId, query, ...extra }
  for (let count = 0; count < 50; count += 1) {
    const page = asOverview(await searchKb(root, request))
    pages.push(page)
...
async function searchAllDetailPages(root: string, kbId: string, query: string, path: string, limit: number): Promise<SearchFileDetailResult[]> {
  const pages: SearchFileDetailResult[] = []
  let request: SearchRequest = { kbId, query, path, limit }
  for (let count = 0; count < 50; count += 1) {
    const page = asFileDetail(await searchKb(root, request))
    pages.push(page)
    if (!page.page.nextCursor) return pages
...
test('overview 返回文件摘要；path 使用知识库根目录相对 POSIX 路径', async () => {
  await withKb('zy-search-overview-', {
    '合同/2024/供应商合同.md': '若乙方违约则解约。\n',
    '会议/纪要.md': '周会无合同。\n',
  }, async (root, kbId) => {
    const result = asOverview(await searchKb(root, {
      kbId,
      query: '违约',
      aliases: ['解约'],
      category: '合同/2024',
    ...
    const detail = asFileDetail(await searchKb(root, {
      kbId,
      query: '违约',
      aliases: ['解约'],
      category: '合同/2024',
...
test('不存在的类目不会静默回退到知识库根目录', async () => {
  await withKb('zy-search-category-', { '会议/纪要.md': '违约金条款。\n' }, async (root, kbId) => {
    await assert.rejects(
      () => searchKb(root, { kbId, query: '违约', category: '没有这个类目' }),
...
test('overview 按文件分页，续页只使用自包含 cursor', async () => {
  await withKb('zy-search-pages-', {
    'b.md': '违约\n',
    'a.md': '违约\n违约\n',
    'c.md': '违约\n',
  }, async (root, kbId) => {
    const pages = await searchAllOverviewPages(root, kbId, '违约', { limit: 1 })
...
test('file-detail 锁定单文件并按命中分页，其他文件不参与结果', async () => {
  ...
  await withKb('zy-search-detail-', {
    'aa/bb/cc.md': `${lines.join('\n')}\n`,
    'other.md': '违约但不应返回。\n',
  }, async (root, kbId) => {
    const pages = await searchAllDetailPages(root, kbId, '违约', 'aa/bb/cc.md', 1)
    assert.equal(pages.length, 3)
...
test('文件详情使用完整单文件扫描策略，概览保持默认上限', async () => {
  await withKb('zy-search-scan-policy-', { 'a.md': '违约。\n' }, async (root, kbId) => {
    ...
    const overview = asOverview(await searchKb(root, { kbId, query: '违约' }, scanner))
    assert.equal(overview.files[0]?.path, 'a.md')
    const detail = asFileDetail(await searchKb(root, { kbId, query: '违约', path: 'a.md' }, scanner))
...
test('正则 OR、类目收窄和文件明细保持同一 path 语义', async () => {
  await withKb('zy-search-regex-', {
    '合同/2024/供应商合同.md': '若乙方违约，甲方可解约。\ntermination 条款见附件。\n',
    '会议/纪要.md': '会议中提到违约，但不在目标类目。\n',
  }, async (root, kbId) => {
    const overview = asOverview(await searchKb(root, {
      kbId,
      query: '违约',
...
test('游标拒绝旧结构和跨 scope 请求，并可单独续页', async () => {
  await withKb('zy-search-cursor-', {
    'a.md': '违约\n',
    'b.md': '违约\n',
  }, async (root, kbId) => {
    const first = asOverview(await searchKb(root, { kbId, query: '违约', limit: 1 }))
    const cursor = first.page.nextCursor
    if (!cursor) throw new Error('缺少下一页游标')
    const second = asOverview(await searchKb(root, { cursor }))
    ...
    await assert.rejects(() => searchKb(root, { cursor: legacyCursor }), /搜索游标无效或已过期/)
    await assert.rejects(
      () => searchKb(root, { cursor, path: 'a.md' } as SearchRequest),
      /续页请求只能包含 cursor 和 limit/,
    )
    await assert.rejects(
      () => searchKb(root, { cursor, query: '解约' } as SearchRequest),
      /续页请求只能包含 cursor 和 limit/,
    )
  })
})
...
test('path 明细的文件不存在、越界和跨类目都会被拒绝', async () => {
  await withKb('zy-search-path-', {
    'a.md': '违约一处。\n',
    'empty.md': '没有相关内容。\n',
    '合同/2024/contract.md': '违约。\n',
  }, async (root, kbId) => {
    const empty = asFileDetail(await searchKb(root, { kbId, query: '违约', path: 'empty.md' }))
    assert.equal(empty.hits.length, 0)
    assert.equal(empty.totalHits, 0)
    await assert.rejects(
      () => searchKb(root, { kbId, query: '违约', path: '没有这个文件.md' }),
      (error: unknown) => error instanceof KbError && error.code === 'not_found',
    )
    await assert.rejects(
      () => searchKb(root, { kbId, query: '违约', path: '../outside.md' }),
      (error: unknown) => error instanceof KbError && error.code === 'path_escape',
    )
    await assert.rejects(
      () => searchKb(root, { kbId, query: '违约', category: '合同/2024', path: 'a.md' }),
      (error: unknown) => error instanceof KbError && error.code === 'path_escape',
    )
    await assert.rejects(
      () => searchKb(root, { kbId, query: '违约', path: '/etc/passwd' }),
      /POSIX 相对路径/,
    )
  })
})
...
test('扫描器不完整时返回 scan.stopReason，不能伪造可续页 cursor', async () => {
  await withKb('zy-search-scan-', { 'a.md': '违约。\n' }, async (root, kbId) => {
    const scanner: SearchScanner = {
      scan: async (input) => {
        assert.equal(input.rootDir, join(root, 'kbs', kbId))
        return {
          matches: [{ path: 'a.md', line: 1, columnByte: 1 }],
          warnings: ['检索结果过多，已截断'],
          complete: false,
    ...
    const result = asOverview(await searchKb(root, { kbId, query: '违约', limit: 1 }, scanner))
    ...
    assert.equal(result.page.hasMore, false)
    assert.equal(result.page.nextCursor, undefined)
    const detail = asFileDetail(await searchKb(root, { kbId, query: '违约', path: 'a.md', limit: 1 }, scanner))
