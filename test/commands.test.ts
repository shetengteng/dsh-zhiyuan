...
describe('kb 斜杠命令', { concurrency: false }, () => {
  ...
  test('ingest 无 --to 时复用上次类目；--root 仍进库根', async () => {
    await withRoot(async (root, run) => {
      ...
      json(await run(`ingest ${first} --base work --to 合同/2024`))
      const reused = json(await run(`ingest ${second} --base work`)) as { copied: string[] }
      assert.ok(reused.copied.includes('合同/2024/b.md'))
      const rooted = json(await run(`ingest ${third} --base work --root`)) as { copied: string[] }
  ...
  test('ingest --to 入队并拷进类目；源文件不改', async () => {
    await withRoot(async (root, run) => {
      ...
      const body = json(await run(`ingest ${src} --base work --to 合同/2024`)) as { copied: string[] }
      assert.ok(body.copied.includes('合同/2024/供应商合同.md'))
      assert.equal(existsSync(join(root, 'bases', 'work', '合同', '2024', '供应商合同.md')), true)
    })
  })
  ...
  test('ingest --root 与 --path；--preserve-tree / --no-create', async () => {
    await withRoot(async (root, run) => {
      ...
      const rooted = json(await run(`ingest --path ${src} --base work --root`)) as { copied: string[] }
      ...
      const preserved = json(await run(`ingest ${join(root, 'src')} --base work --to 归档 --preserve-tree`)) as {
        copied: string[]
      }
      assert.ok(preserved.copied.some((item) => item.includes('子/b.md')))

      const missing = await run(`ingest ${src} --base work --to 尚不存在 --no-create`)
  ...
  test('search：rest 作 query，--aliases 中英文逗号，--to 收窄类目', async () => {
    await withRoot(async (root, run) => {
      ...
      await writeFile(join(root, 'bases', 'work', '合同', '2024', '供应商合同.md'), '若乙方违约则解约。\n')
      await writeFile(join(root, 'bases', 'work', '会议', '纪要.md'), '周会无合同。\n')
      const result = json(await run('search 违约 --base work --aliases 解约，termination --to 合同/2024')) as {
...
describe('kb call', { concurrency: false }, () => {
  test('list / create / update / deleteBase', async () => {
    await withRoot(async (_root, run) => {
      ...
      const updated = json(await run(callLine({ op: 'update', id: 'work', title: '公司库' }))) as { title: string; id: string }
      assert.equal(updated.id, 'work')
      assert.equal(updated.title, '公司库')
      const denied = await run(callLine({ op: 'deleteBase', id: 'work' }))
      assert.equal(denied.kind, 'error')
      assert.match(denied.text ?? '', /确认/)
      assert.deepEqual(json(await run(callLine({ op: 'deleteBase', id: 'work', confirm: true }))), { ok: true })
      assert.deepEqual(json(await run(callLine({ op: 'list' }))), [])
    })
  })
  ...
  test('tree / read / write / deleteEntry', async () => {
    await withRoot(async (_root, run) => {
      await createBase(_root, { id: 'work', title: '工作库', description: '描述' })
      assert.deepEqual(json(await run(callLine({ op: 'write', id: 'work', path: '合同/2024/a.md', text: 'hello' }))), { ok: true })
      const entry = json(await run(callLine({ op: 'read', id: 'work', path: '合同/2024/a.md' }))) as { text: string }
      assert.equal(entry.text, 'hello')
      const tree = json(await run(callLine({ op: 'tree', id: 'work' }))) as Array<{ name: string; kind: string }>
      assert.equal(tree[0].name, '合同')
      assert.equal(tree[0].kind, 'dir')
      const denied = await run(callLine({ op: 'deleteEntry', id: 'work', path: '合同/2024/a.md' }))
      assert.equal(denied.kind, 'error')
      assert.deepEqual(json(await run(callLine({ op: 'deleteEntry', id: 'work', path: '合同/2024/a.md', confirm: true }))), { ok: true })
      const missing = await run(callLine({ op: 'read', id: 'work', path: '合同/2024/a.md' }))
      assert.equal(missing.kind, 'error')
  ...
  test('prefs / setPrefs 只改给出的字段', async () => {
    await withRoot(async (_root, run) => {
      await run(callLine({ op: 'create', id: 'work', title: '工作库', description: '描述' }))
      const prefs = json(await run(callLine({ op: 'prefs' }))) as { defaultBaseId: string; maxFileBytes: number }
      assert.equal(prefs.defaultBaseId, 'work')
      const updated = json(await run(callLine({ op: 'setPrefs', maxFileBytes: 1024, defaultBaseId: 'work' }))) as {
        maxFileBytes: number
        defaultBaseId: string
        maxBaseBytes: number
      }
      assert.equal(updated.maxFileBytes, 1024)
      assert.equal(updated.defaultBaseId, 'work')
  ...
  test('resolveIngestTo：--to 优先，--root 次之，否则上次类目', async () => {
    await withRoot(async (root) => {
      await createBase(root, { id: 'work', title: '工作库', description: '描述' })
      assert.equal(await resolveIngestTo(root, 'work', '合同/2024', false), '合同/2024')
      assert.equal(await resolveIngestTo(root, 'work', undefined, true), '')
      await assert.rejects(() => resolveIngestTo(root, 'work', undefined, false), /--to/)
    })
  })
